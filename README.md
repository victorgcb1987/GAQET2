...
The inner ring represents the **area calculated from the radar plot** for each annotation represented. Area value is also represented as a numerical value on the legend. This is an **annotation quality's global score** and higher means better.  
Columns sets are representing **in-depth annotation metrics**:  
BUSCO sections: Higher % complete, Single means better. If there are no valid biological explanations, for example poliploidy, higher values of % Complete, Duplicated could be indicative of assembly problems, for example redundant contigs generated by pooling DNA samples of multiple individuals. High levels of % Fragemented could be an indicative of a Genome's assembly low continuity, but 
