...
- **Column sets**:  
  ...
  - **Transposons**: Here is represented the % of transcript models classified as PTeMte (potential TEs). Usually, higher values means worse, but sometimes this sequences are flagged as TEs because actually they are genes originated from TEs domestication. Check DeTEnGA output file ```{species}_TE_summary.csv```for a list of sequences IDs for further analysis on this sequences
